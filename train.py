...
def train_model():
    ...
    train_dataset = datasets.ImageFolder(
        root=dataset_dir, transform=data_transforms)
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_data, batch_size=bsz, shuffle=True, num_workers=4)
    val_dataloader = torch.utils.data.DataLoader(
        val_data, batch_size=bsz, shuffle=False, num_workers=4)
    ...
    model_ft = models.efficientnet_v2_s(
        num_classes=TRAIN_MODE.get("anms")).to(device)
    model_ft.classifier[1] = nn.Linear(
        model_ft.classifier[1].in_features, TRAIN_MODE.get("anms")).to(device)
    ...
    try:
        for epoch in range(epochs):
            ...
            print(
                f"Train Loss: {epoch_loss:.4f}, Train Acc: {epoch_accuracy:.4f}, Time: {time.time() - start:.2f}s")
            model_ft.eval()
            with torch.no_grad():
                test_epoch_loss = 0
                ...
                for test_X, test_y in tqdm(val_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Validation"):
                    ...
                    test_accuracy = (test_preds.argmax(
                        dim=1) == test_y).float().mean()
                    test_epoch_accuracy += test_accuracy.item()
                test_epoch_accuracy /= len(val_dataloader)
                test_epoch_loss /= len(val_dataloader)
                print(
                    f"Test Loss: {test_epoch_loss:.4f}, Test Acc: {test_epoch_accuracy:.4f}, Time: {time.time() - start:.2f}s")
